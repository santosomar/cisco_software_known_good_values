...
def convert_cert_to_pem(cert_name):
    with open(cert_name, 'r') as f:
        ...
        if(first_line.find("-----BEGIN CERTIFICATE-----") == -1):
            cmd = "openssl x509 -inform der -in {0} -out {1}".format(cert_name, cert_name)
...
def verify_3tier_cert_chain(ee_cert):
    ...
    cmd = "openssl verify -CAfile {0} {1}".format(root_cert, subca_cert)
    status, out = commands.getstatusoutput(cmd)
    ...
    cmd = "openssl verify -CAfile {0} -untrusted {1} {2}".format(root_cert, subca_cert, ee_cert)
    status, out = commands.getstatusoutput(cmd)
...
def fetch_pubkey_from_cert(cert_name):
    cmd = "openssl x509 -pubkey -noout -in {0} > ee_pubkey.pem".format(cert_name)
    status, out = commands.getstatusoutput(cmd)
...
def verify_dgst_signature(args):
    ...
    cmd = "openssl dgst -{0} -verify ee_pubkey.pem -signature {1} {2}".format(sha_version, args.signature, args.image_name)
    status, out = commands.getstatusoutput(cmd)
...
def verify_smime_signature(args):
    cmd = "openssl smime -verify -binary -in {0} -inform PEM -content {1} -noverify -nointern -certfile {2}".format(args.signature, args.image_name, args.ee_cert)
    status, out = commands.getstatusoutput(cmd)
